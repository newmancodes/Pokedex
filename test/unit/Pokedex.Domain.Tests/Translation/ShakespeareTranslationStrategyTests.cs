...
namespace Pokedex.Domain.Tests.Translation
{
    public class ShakespeareTranslationStrategyTests
    {
        ...
        public async Task Successful_Translations_Are_Returned()
        {
            ...
            translationService.Translate(value, "shakespeare", cancellationTokenSource.Token).Returns(translatedValue);
            var sut = new ShakespeareTranslationStrategy(translationService, Substitute.For<ILoggerFactory>());
        ...
        public async Task Unsuccessful_Translations_Are_Expressed()
        {
            ...
            var value = "some_value";
            var translationService = Substitute.For<ITranslationService>();
            translationService.Translate(value, "shakespeare", cancellationTokenSource.Token).Throws(new Exception("some_reason_for_failure"));
            var sut = new ShakespeareTranslationStrategy(translationService, Substitute.For<ILoggerFactory>());
